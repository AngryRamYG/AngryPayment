    ...
    public partial class Index
    {
        ...
        public async Task ProcessPaymentAsync()
        {
            ...
            if (isNew)
            {
                ...
                //cant we check for this using the binding parameters?
        ...
        private async Task CheckPaymentMethod(PaymentIntent paymentIntent, bool newClient, PaymentIntentService service)
        {
            try
            {
                ...
                if (payment.error == null && payment.paymentintent.status == "succeeded")
                {
                    ...
                    Description = $"Payment succeeded <br/> Amount Paid:{(balance.Amount)*1.0 / 100.0} <br/> Net Fee : {(balance.Net)*1.0 / 100.0} <br/> Fee:{(balance.Fee)*1.0 / 100.0}";
                    submitted = true;
